...
export function fromAutoTrader(doc: Document): Partial<Vehicle> {
  const title = doc.querySelector('h1, .vehicle-title')?.textContent || '';
  const yearMakeModel = title.match(/(\d{4})\s+([A-Za-z][^\s]+)\s+(.+)/);
  const priceText =
    (doc.querySelector('[data-cmp="firstPrice"], [data-test="vdp-price"]') as HTMLElement | null)
      ?.innerText || '';
  const mileageText = (doc.querySelector('[data-cmp="mileage"]') as HTMLElement | null)?.innerText || '';
  ...
  const dealerName =
    (doc.querySelector('[data-cmp="sellerName"]') as HTMLElement | null)?.innerText || undefined;

  const cityStateZip =
    (doc.querySelector('[data-cmp="sellerAddress"]') as HTMLElement | null)?.innerText || '';

  const out: Partial<Vehicle> = {
    year: yearMakeModel ? parseInt(yearMakeModel[1], 10) : undefined,
    make: yearMakeModel ? yearMakeModel[2] : undefined,
    model: yearMakeModel ? yearMakeModel[3] : undefined,
    ...
    description: (doc.querySelector('[data-cmp="description"]') as HTMLElement | null)?.innerText || undefined,
  };
