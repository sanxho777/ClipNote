...
async function runScrape() {
  const result: ScrapeResult = await scrapeCurrent(document);
  chrome.runtime.sendMessage({ type: 'VSML_SCRAPE_RESULT', payload: result }, () => {});
  // Also stash it on the page for immediate popup retrieval
  (window as any).__VSML_LAST_SCRAPE__ = result;
