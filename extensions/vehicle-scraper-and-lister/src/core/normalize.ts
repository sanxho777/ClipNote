import type { Vehicle, Photo } from '../types';

function toNumber(n?: string | null): number | undefined {
...
export function normalizeVehicle(v: Vehicle): Vehicle {
  const out: Vehicle = { ...v };
  out.price = v.price ?? toNumber(v.price as any);
  out.mileage = v.mileage ?? toNumber(v.mileage as any);
  out.year = v.year ?? toNumber(v.year as any);
  out.make = titleCase(v.make);
  out.model = titleCase(v.model);
  out.trim = v.trim ? v.trim.trim() : undefined;
...
export function pickLargestSrc(img: HTMLImageElement): string | undefined {
...
export function vinFromText(text: string): string | undefined {
  const m = text.match(/\b[A-HJ-NPR-Z0-9]{17}\b/i);
  return m?.[0]?.toUpperCase();
}

export function ensureFilenames(photos: Photo[]): Photo[] {
