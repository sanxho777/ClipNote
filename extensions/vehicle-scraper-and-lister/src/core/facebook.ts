...
export async function waitForSelector(sel: string, timeout = 15000): Promise<Element> {
...
export async function autofillOnFacebook(v: Vehicle): Promise<void> {
  ...
  // Price
  const priceInput = document.querySelector('input[aria-label*="Price" i]') as HTMLInputElement | null
    || document.querySelector('input[name="price"]') as HTMLInputElement | null;
  if (priceInput && typeof v.price === 'number') setInputValue(priceInput, String(v.price));

  // Mileage
  const milesInput = document.querySelector('input[aria-label*="Mileage" i]') as HTMLInputElement | null
    || document.querySelector('input[name="mileage"]') as HTMLInputElement | null;
  if (milesInput && typeof v.mileage === 'number') setInputValue(milesInput, String(v.mileage));

  // VIN
  const vinInput = document.querySelector('input[aria-label*="VIN" i]') as HTMLInputElement | null
    || document.querySelector('input[name="vin"]') as HTMLInputElement | null;
  if (vinInput && v.vin) setInputValue(vinInput, v.vin);

  // Condition
  const conditionBtn = Array.from(document.querySelectorAll('[role="button"]')).find((b) =>
    (b as HTMLElement).innerText?.match(/(Condition|New|Used)/i)
  ) as HTMLElement | undefined;
  if (conditionBtn) {
    conditionBtn.click();
    await new Promise((r) => setTimeout(r, 200));
    const used = Array.from(document.querySelectorAll('*')).find((el) =>
      (el as HTMLElement).innerText?.match(/^Used$/i)
    ) as HTMLElement | undefined;
    used?.click();
  }

  // Structured vehicle data
  async function setSelectLike(labelRe: RegExp, value: string | number | undefined) {
    if (!value) return;
    ...
    const button = (container.querySelector('[role="button"]') ||
      container.querySelector('[aria-haspopup="listbox"]')) as HTMLElement | null;
    (button as HTMLElement | null)?.click();
    await new Promise((r) => setTimeout(r, 200));
    const opt = Array.from(document.querySelectorAll('[role="option"], [role="menuitem"]')).find((el) =>
      (el as HTMLElement).innerText.toLowerCase().includes(String(value).toLowerCase())
    ) as HTMLElement | undefined;
    opt?.click();
  }
  ...
  if (v.dealer?.zip || v.dealer?.city) {
    const locInput =
      (document.querySelector('input[aria-label*="Location" i]') as HTMLInputElement | null) ||
    ...
    if (locInput && locText) {
      ...
      const first = document.querySelector('[role="option"], [role="menuitem"]') as
        | HTMLElement
        | null;
      first?.click();
    }
  ...
  const descArea =
    (document.querySelector('textarea[aria-label*="Description" i]') as HTMLTextAreaElement | null) ||
...
export function buildDescription(v: Vehicle): string {
