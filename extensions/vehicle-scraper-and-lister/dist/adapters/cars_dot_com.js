...
export function fromCarsDotCom(doc)=
    (doc.querySelector('h1[data-testid="structured-data-title"]') | null)?.innerText ||
    (doc.querySelector('h1') | null)?.innerText ||
    '';

  const match = title.match(/(\d{4})\s+([A-Za-z][^\s]+)\s+(.+)/);
  const priceText = (doc.querySelector('[data-testid="price-section"] [data-testid="price"]') HTMLElement
    | null)?.innerText;
  const mileageText = (doc.querySelector('[data-testid="mileage"]') | null)?.innerText;

  const specs = Array.from(doc.querySelectorAll('[data-testid="specifications-section"], table, ul li'))
    .map((el) => el.textContent || '');
  const out= {
    year: match ? parseInt(match[1], 10) ,
    make: match ? match[2] ,
    model: match ? match[3] ,
    price: priceText ? parseInt(priceText.replace(/[^\d]/g, ''), 10) ,
    mileage: mileageText ? parseInt(mileageText.replace(/[^\d]/g, ''), 10) ,
    description: (doc.querySelector('[data-testid="seller-notes"]') | null)?.innerText || undefined,
  };
  ...
  // Dealer block
  const dealerName = (doc.querySelector('[data-qa="dealer-name"]') | null)?.innerText;
  const address = (doc.querySelector('[data-qa="dealer-address"]') | null)?.innerText || '';
  out.dealer = {
    name,
